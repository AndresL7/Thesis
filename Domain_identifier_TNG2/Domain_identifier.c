...
int main(int argc, char *argv[])
{
  ...
  int i, Tini, counter, NGROUPS=0;
  int ihalo, istart, iend, Nparts_in_node;
  ...
  // TNG
  data_prefix    = argv[1];
  catalog_prefix = argv[2];
  
  snprintf(infile,   MAX_FILENAME, "%s.%d.hdf5", data_prefix,    0);
  snprintf(cat_file, MAX_FILENAME, "%s.%d.hdf5", catalog_prefix, 0);
  ...
  GalCos_preload_Catalog(cat_file);
  ...
  if(task == 0)
      ...
      for(i=0; i<Number_Processes; i++)
	{
      ...
      domain_info[i].iend = UNCLUSTERED+Npart_clustered;
      domain_info[i].Nparts_per_node = domain_info[i].iend - domain_info[i].istart;
  ...
  /////////////////////////////////////////////////////////////////////////////////
  /*                  Building spatial grid for halo search                      */
  /////////////////////////////////////////////////////////////////////////////////
  
  if(task == 0)
    printf(" >> Building spatial grid for optimized halo search...\n"); fflush(stdout);
  
  build_halo_spatial_grid();
  ...
  counter=0;
  if (task ==0)
    printf("Aprox number of particles per node after resampling: %d\n", domain_info[task].Nparts_per_node*1);

  srand(time(NULL) + task);
  ...
  for(i=0; i<domain_info[task].Nparts_per_node; i++)
	...
	printf(" (%d) *Computed domains for %d at %f hours\n",task,counter,
	       (tiempoc()-Tini)/3600.0); fflush(stdout);
  ...
  if(task == 0) 
      ...
      for(j=0; j<NCLUSTERS; j++)
	{
	  fprintf(pf,"%16.8lf %16.8lf %16.8lf %16.8lf %16.8lf %16.8lf %12d\n", 
		  ...
		  Halos[j].Mvir, Halos[j].Rvir, Halos[j].NDomain_particles*PARTMASS, 
		  Halos[j].NDomain_particles);
	}
      fclose(pf);
    }
  
  // Liberar memoria del grid espacial
  free_halo_spatial_grid();
